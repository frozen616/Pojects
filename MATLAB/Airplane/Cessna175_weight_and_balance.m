...
prompt = 'What is the luggage area B weight in lbs? ';
bag_b = input(prompt)
...
arm_bew = 37.4;
arm_pilot_front_pass = 37.0;
arm_rear_pass = 73.0;
arm_fuel = 48.0;
arm_bag_a = 95.0;
arm_bag_b = 123.0;

ramp_weight = bew + pilot_front_pass + rear_pass + fuel_weight + bag_a + bag_b;
...
mom_bag_b = arm_bag_b .* bag_b;

ramp_moment = mom_bew + mom_pilot + mom_rear_pass + mom_fuel + mom_bag_a + mom_bag_b;
...
syms x
y = piecewise(x<35 , 0 , 35<x<38.5 , (x*100)-1550 , 38.5<x<47.25 , 2300 , x>47.25 , 0);
fplot(y)
hold on
syms p
